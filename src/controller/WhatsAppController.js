...
export class WhatsAppController {
    ...
    */
    ...
    setAtctivChat(contact){

        if(this._contactActive){
            Message.getRefMSN(this._contactActive.chatId)
        ...
        let containerMSG = this.el.panelMessagesContainer;
        containerMSG.innerHTML = '';
        Message.getRefMSN(this._contactActive.chatId)
        .orderBy('timeStamp')
        .onSnapshot(docs=>{
            let scrollTop = containerMSG.scrollTop;
            let scrollTopMax = (containerMSG.scrollHeight - containerMSG.offsetHeight);
            ...
            docs.forEach(doc=>{
                ...
                if(this.el.panelMessagesContainer.querySelector('#_'+data.id) && me){
                    ...
                    msgEl.querySelector('.message-status').innerHTML = msg.getStatusElementView().outerHTML;
                }

                if(msg.type === 'contact'){
                    view.querySelector('.btn-message-send').on('click', e=>{

                        Chat.createIfNotExists(this._user.email, msg.content.email).then(chat=>{

                            let contact = new User(msg.content.email);
            ...
            if(autoScroll){
                ...
                (containerMSG.scrollHeight - containerMSG.offsetHeight)
            } else{
                containerMSG.scrollTop = scrollTop;
            }
        })
    }
    ...
    initEvents() {
        ...
        this.el.inputProfilePhoto.on('change', e=>{
            if(this.el.inputProfilePhoto.files.length > 0){
                ...
                Message.uploadHD(file, this._user.email).then(snapshot=>{
        ...
        this.el.btnSendPicture.on('click', e=>{
            ...
            picture.onload = event=>{
                ...
                fetch(canvas.toDataURL(mimeType)).then(
                    res=>{ return res.arrayBuffer();}
                ).then(
                    buffer=>{return new File([buffer], fileName, { type: mimeType});}
                ).then(
                    file =>{ Message.sendImage(this._contactActive.chatId, this._user.email, file);
        ...
        this.el.btnSendDocument.on('click', e=>{
            ...
            let file = this.el.inputDocument.files[0];
            let base64El = this.el.imgPanelDocumentPreview.src;

            if(file.type === 'application/pdf'){

                Base64.toFile(base64El).then(filePreview=>{
        ...
        this.el.btnAttachContact.on('click', e=>{
            ...
            this._contactController.on('select', contacts=>{
        ...
        this.el.btnFinishMicrophone.on('click', e=>{
            ...
            this._microphoneController.on('recorded', (file, mdata)=>{
                Message.sendAudio(this._contactActive.chatId, 
                    file, mdata, this._user.email, this._user.photo,'audio')
            });
        ...
        this.el.btnSend.on('click', e=>{
            let text = this.el.inputText;

            Message.sendMessage(this._contactActive.chatId, 
                text.innerHTML,
                this._user.email, 
                "text");
